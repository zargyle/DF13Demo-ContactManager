...
ContactManagerApp = (function(Backbone, Marionette) {
	...
	*/
	...
	App.addInitializer(function(options) {
		App.layout = new ContactManagerApp.ContactManagerAppLayout();
	...
	App.on("initialize:after", function(options) {
		...
		App.startModule("TheList", {
			region: App.layout.list
		});

		App.listenTo(App.vent,"EditContact",function(options){
			App.startModule("TheForm", {
				region: App.layout.form
			});
			App.module("TheForm").controller.loadContact({recordId:options.recordId});
		});

		App.listenTo(App.vent,"ContactSaveSuccessful",function(options){
			App.stopModule("TheForm",{
				region: App.layout.form
			});
			App.module("TheList").controller.loadContacts();
		});
...
ContactManagerApp.AppController = (function(App, Marionette) {
...
})(ContactManagerApp, Marionette);
window.runningApp = ContactManagerApp;//set the running application